...
public class ImageUnion {
	 ...
	 * 存放待结合图片的目录
	 */
//	private static String srcpath = "C:/Documents and Settings/weiwei/My Documents/tmpPict";
	private static String srcpath = "/Users/lilin/Desktop/sample";

	/**
	 * 结合后的图片
	 */
//	private static String output = "C:/Documents and Settings/weiwei/My Documents/myDb/temp.png";
	private static String output = "/Users/lilin/Desktop/result.png";
	...
	/**
	 * 结合图片给定的图片集
	 * @param destFile 结合结果文件
	 * @param height
	 * @param quatangles 图片的大小及放置的位置
	 * @throws Exception
	 */
	public void union(String destFile, int height, List<Quatangle> quatangles)
			throws Exception {
		
		BufferedImage thumbnail = new BufferedImage(FINAL_WIDTH, height, BufferedImage.TYPE_INT_RGB);
		...
		for (Quatangle size : quatangles) {
			...
			// 旋转
			file = (AUTO_ROTATE && size.getDirectory() == 6) ? rotate90ToRight(file) : file;
			file = (AUTO_ROTATE && size.getDirectory() == 3) ? rotate90ToRight(rotate90ToRight(file)) : file;
			...
			// 在结果画板上绘制调整后的图片
			graphics.drawImage(workimage, (int) size.getX(), (int) size.getY(), null);
	...
	/**
	 * 确定每张图片的坐标与显示大小
	 * @param tmpQuant 输入的图片信息
	 * @param threeDim 图片分类的结果，最外层代表所有图片，中间代表行，最里层是每一行的图片的标识（即图片顺序）
	 * @return
	 */
	public int setCoordinate(List<Quatangle> tmpQuant, Vector<Vector<Vector<Integer>>> threeDim) {
		...
		while (threeDim.size() > 0) {
			...
			int TEMP_WIDTH = FINAL_WIDTH - (threeDim.get(tmpIndex).size() + 1) * INTER_WIDTH;
			
			
			for (int i = 0; i < threeDim.get(tmpIndex).size(); i++) {
				double tmpProp = 0.0;
				for (int j = 0; j < threeDim.get(tmpIndex).get(i).size(); j++) {
					tmpProp = tmpProp + 1/tmpQuant.get(threeDim.get(tmpIndex).get(i).get(j)).getProp();
				}
			...
			while (threeDim.get(tmpIndex).size() > 0) {
				...
				int subIndex = (int)(Math.random() * threeDim.get(tmpIndex).size());
				int sumSubY = 0;
				
				// 最里层Vector，横向
				if (threeDim.get(tmpIndex).size() == 1) {
					if (threeDim.get(tmpIndex).get(subIndex).size() == 1) {
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setX(sumSubX);
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setY(sumY);
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setEdge(FINAL_WIDTH - sumSubX - INTER_WIDTH, (int) (TEMP_WIDTH/sumProp));
					}
					// 最里层Vector，纵向
					if (threeDim.get(tmpIndex).get(subIndex).size() > 1) {
						while (threeDim.get(tmpIndex).get(subIndex).size() > 0) {
							int ssIndex = (int)(Math.random() * threeDim.get(tmpIndex).get(subIndex).size());
							if (threeDim.get(tmpIndex).get(subIndex).size() == 1) {
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setX(sumSubX);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setY(sumY + sumSubY);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setEdge(FINAL_WIDTH - sumSubX - INTER_WIDTH, ((int) (TEMP_WIDTH/sumProp)) - sumSubY);
							}
							else {
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setX(sumSubX);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setY(sumY + sumSubY);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setEdge(FINAL_WIDTH - sumSubX - INTER_WIDTH, ((int) ((FINAL_WIDTH - sumSubX - INTER_WIDTH)/tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).getProp())));
								sumSubY = sumSubY + ((int) ((FINAL_WIDTH - sumSubX - INTER_WIDTH)/tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).getProp())) + INTER_WIDTH;
							}
							threeDim.get(tmpIndex).get(subIndex).remove(ssIndex);
						}
					}
				}
				else {
					if (threeDim.get(tmpIndex).get(subIndex).size() == 1) {
						double subProp = tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).getProp();
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setX(sumSubX);
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setY(sumY);
						tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(0)).setEdge((int) ((subProp/sumProp) * TEMP_WIDTH), (int) (TEMP_WIDTH/sumProp));
						sumSubX = sumSubX + ((int) ((subProp/sumProp) * TEMP_WIDTH)) + INTER_WIDTH;
					}
					//最里层Vector，纵向
					if (threeDim.get(tmpIndex).get(subIndex).size() > 1) {
						double subProp = 0.0;
						for (int k = 0; k < threeDim.get(tmpIndex).get(subIndex).size(); k++) {
							subProp = subProp + 1/tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(k)).getProp();
						}
						subProp = 1.0 / subProp;
						while (threeDim.get(tmpIndex).get(subIndex).size() > 0) {
							int ssIndex = (int)(Math.random() * threeDim.get(tmpIndex).get(subIndex).size());
							...
							if (threeDim.get(tmpIndex).get(subIndex).size() == 1) {
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setX(sumSubX);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setY(sumY + sumSubY);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setEdge((int) ((subProp/sumProp) * TEMP_WIDTH), ((int) (TEMP_WIDTH/sumProp)) - sumSubY);
							}
							else {
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setX(sumSubX);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setY(sumY + sumSubY);
								tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).setEdge((int) ((subProp/sumProp) * TEMP_WIDTH), (int) (((subProp/sumProp) * TEMP_WIDTH)/tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).getProp()));
								sumSubY = sumSubY + ((int) (((subProp/sumProp) * TEMP_WIDTH)/tmpQuant.get(threeDim.get(tmpIndex).get(subIndex).get(ssIndex)).getProp())) + INTER_WIDTH;
							}
		...
		if (twoIndex.get(0).size() > 0) {
			...
			if (section1.size() <= (PropA_MAX_UN * (section1.size() / PropA_MIN_UN))) {
				...
				if ((sectionLen / PropA_MIN_UN - sectionLen / PropA_MAX_UN) > 1) {
					rowcount = (sectionLen/PropA_MIN_UN + sectionLen/PropA_MAX_UN)/2;
				}
	 ...
	 * @param tmpQuant 原始图片集
	 * @param twoIndex 分类好的结果集
	 */
	public List<List<Integer>> divideSection(List<Quatangle> tmpQuant) {
		...
		for (int i = 0; i < tmpQuant.size(); i++) {
			...
			if (tmpQuant.get(i).getProp() > limitPropA && tmpQuant.get(i).getProp() <= limitPropB) {
				section2.add(i);
			}
			if (tmpQuant.get(i).getProp() > limitPropB && tmpQuant.get(i).getProp() <= limitPropC) {
				section3.add(i);
			}
